...
def _parse_blend_filename_for_report(filepath):
    """Extrae name y branch de un .blend para inferir el nombre del reporte CSV."""
...
# Función segura para rutas relativas
def safe_relative_path(target, base):
    """Calcula una ruta relativa segura, incluso para rutas de red"""
    try:
        return os.path.relpath(target, base)
    except ValueError:
        # Manejo de rutas UNC (Windows network paths)
        if target.startswith('\\\\') and base.startswith('\\\\'):
            target_parts = target.split('\\')[2:]
            base_parts = base.split('\\')[2:]
            common = 0
            min_len = min(len(target_parts), len(base_parts))
            for i in range(min_len):
                if target_parts[i].lower() != base_parts[i].lower():
                    break
                common += 1
            # Construir la ruta relativa
            up_dirs = ['..'] * (len(base_parts) - common)
            rel_parts = up_dirs + target_parts[common:]
            return '\\'.join(rel_parts)
        # En otros casos, devolvemos la ruta absoluta
        return target
...
class BPM_MissingReportItem(bpy.types.PropertyGroup):
    path: bpy.props.StringProperty()

class BPM_OverseerSettings(bpy.types.PropertyGroup):
    csv_paths: bpy.props.CollectionProperty(type=BPM_OverseerCSV)
    scan_directory: bpy.props.StringProperty(
        name="Root Directory",
        description="Select the root directory to scan for CSV reports",
        subtype='DIR_PATH'
    )
    missing_reports: bpy.props.CollectionProperty(type=BPM_MissingReportItem)
    show_missing_reports: bpy.props.BoolProperty(name="Missing Reports", default=True)
...
class PM_OT_OpenFileBrowser(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        def _collect_paths(parent, struct):
            ...
            items = struct.items()
            if ordered: items = [(k, struct[k]) for k in ordered] + sorted([(k, v) for k, v in struct.items() if k not in ordered])
...
#  OPERATORS 
# ====================================================================================
class BPM_OT_SearchAndLoadReports(bpy.types.Operator):
    bl_idname = 'bpm.search_and_load_reports'
    bl_label = 'Search and Load CSV Reports'
    bl_description = 'Scans the selected directory and subdirectories for .csv files and loads them into the Overseer'

    def execute(self, context):
        settings = context.scene.bpm_overseer
        root_dir = bpy.path.abspath(settings.scan_directory)
        
        if not root_dir or not os.path.isdir(root_dir):
            self.report({'WARNING'}, 'Please select a valid root directory first.')
            return {'CANCELLED'}
        
        store = settings.csv_paths
        existing_paths = {e.path for e in store}
        found_count = 0
        newly_added_paths = []
            ...
            for file in files:
                if file.lower().endswith('.csv'):
                    ...
                    if full_path not in existing_paths:
                        newly_added_paths.append(full_path)
        ...
        if newly_added_paths:
            for path in newly_added_paths:
                ...
                new_entry.rows.clear()
                for rec in read_csv_report(path):
                    row = new_entry.rows.add()
                    row.name = rec.get("Name", "")
                    row.branch = rec.get("Branch", "")
                    row.type = rec.get("Type", "")
                    row.last_update = rec.get("LastUpdate", "")
                    row.process = rec.get("Process", "")
                    row.state = rec.get("State", "")
                    row.assigned_to = rec.get("AssignedTo", "")
                    row.note = rec.get("Note", "")
                    row.checked = rec.get("Checked", "false").lower() == "true"
                ...
                for area in window.screen.areas:
                    if area.type == 'VIEW_3D':
                        for region in area.regions:
                            if region.type == 'UI':
                                region.tag_redraw()
            self.report({'INFO'}, f'Found and loaded {found_count} new CSV reports.')
        else:
            self.report({'INFO'}, 'No new CSV reports found.')
            
        return {'FINISHED'}


class BPM_OT_FindMissingReports(bpy.types.Operator):
    bl_idname = 'bpm.find_missing_reports'
    bl_label = 'Find Missing Reports'
    bl_description = 'Scan for .blend files that do not have a corresponding .csv report (ignores versioned files)'
    ...
    def execute(self, context):
        ...
        settings.missing_reports.clear()
        found_missing_count = 0

        # Expresión regular para detectar versiones progresivas (_001, _002, etc.)
        ...
        for subdir, _, files in os.walk(root_dir):
            for file in files:
                if file.lower().endswith('.blend'):
                    ...
                    # Si es un archivo de versión progresiva (_001, _002, etc.), lo ignoramos
                    if version_pattern.search(base_name):
                        continue
                    
                    name, branch = _parse_blend_filename_for_report(file)
                    if name is None:
                        continue
                    
                    csv_name = f"{branch}_{name} report.csv" if branch != "00" else f"{name} report.csv"
                    expected_csv_path = os.path.join(subdir, csv_name)

                    if not os.path.exists(expected_csv_path):
                        blend_path = os.path.join(subdir, file)
                        # Normalizamos la ruta del archivo .blend
                        blend_path = os.path.normpath(blend_path)
                        item = settings.missing_reports.add()
                        item.path = blend_path
                        found_missing_count += 1

        if found_missing_count > 0:
            self.report({'INFO'}, f'Found {found_missing_count} .blend file(s) with missing reports.')
            settings.show_missing_reports = True
        else:
            self.report({'INFO'}, 'All .blend files have a corresponding report.')
            settings.show_missing_reports = False
...
class BPM_OT_AddCSV(bpy.types.Operator):
    bl_idname = 'bpm.add_csv'; bl_label = 'Load Single Reports'
...
class BPM_PT_Overseer(bpy.types.Panel):
    ...
    def draw(self,ctx):
        ...
        auto_box = lay.box()
        auto_box.label(text="Auto Scan", icon='AUTO')
        auto_box.prop(data, "scan_directory", text="")
        
        search_row = auto_box.row(align=True)
        search_row.enabled = bool(data.scan_directory)
        search_row.operator('bpm.search_and_load_reports', text="Search for Reports", icon='VIEWZOOM')
        search_row.operator('bpm.find_missing_reports', text="Missing Reports", icon='ERROR')
...
# ====================================================================================
#  PANEL Missing Reports
# ====================================================================================
class BPM_PT_MissingReportsPanel(bpy.types.Panel):
    bl_idname = "BPM_PT_MissingReportsPanel"
    bl_label = "Missing Reports List"
    ...
    def poll(cls, context):
        return context.scene.bpm_overseer and context.scene.bpm_overseer.missing_reports
    ...
    def draw(self, context):
        ...
        box = layout.box()
        row = box.row()
        row.prop(settings, "show_missing_reports", 
                 icon='TRIA_DOWN' if settings.show_missing_reports else 'TRIA_RIGHT', 
                 icon_only=True, emboss=False)
        row.label(text=f"Found: {len(settings.missing_reports)}")

        if settings.show_missing_reports:
            col = box.column(align=True)
            for item in settings.missing_reports:
                # Usamos nuestra función segura para rutas relativas
                rel_path = safe_relative_path(item.path, settings.scan_directory)
                
                # Mostramos el nombre del archivo y la ruta relativa
                row = col.row()
                row.label(text=os.path.basename(item.path), icon='BLENDER')
                row = col.row()
                row.label(text=rel_path)
...
# ====================================================================================
CLASSES = [
    BPM_OverseerRow, BPM_OverseerCSV, BPM_MissingReportItem, BPM_OverseerSettings,
    PM_OT_OpenFileBrowser,
    BPM_OT_SearchAndLoadReports,
    BPM_OT_FindMissingReports,
    BPM_OT_AddCSV, BPM_OT_RemoveCSV, BPM_OT_ReloadCSV, BPM_OT_ReloadAll,
    BPM_OT_ValidateAsset, BPM_OT_ValidateAll, BPM_OT_ExportCombined, 
    BPM_PT_Overseer,
    BPM_PT_MissingReportsPanel,
